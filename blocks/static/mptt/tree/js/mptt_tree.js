...
mptt.jQuery(function($){
    ...
    {
        return mptt.expanded_nodes.indexOf(id) !== -1;
    }

    function markNodeAsExpanded(id) {
        if (!isExpandedNode(id))
            mptt.expanded_nodes.push(id);
    }

    function markNodeAsCollapsed(id) 
    {
        // remove itemId from array of collapsed nodes
        var idx = mptt.expanded_nodes.indexOf(id);
        if(idx >= 0)
            mptt.expanded_nodes.splice(idx, 1);
    }

    // toggle children
    function doToggle(id, show) {
        var children = mptt.tree_structure[id];
    ...
    /*
     * mptt Drag-n-drop tree reordering.
    ...
    $.extend($.fn.mpttTree = function() {
        $('tr', this).each(function(i, el) {
            ...
            if (mptt.tree_structure[pageId].length) {
        ...
        $('div.drag_handle').bind('mousedown', function(event) {
            ...
            $("body").addClass('dragging').disableSelection().bind('mousemove', function(event) {
                ...
                var cloned = '<table>' + originalRow.html() + '</table>';
                ...
                if ($('#ghost').length === 0)
                {
                    ...
                    $('#ghost table').css({
                        'width': originalRow.closest('table').outerWidth(),
                        'height': originalRow.outerHeight()
                    });
                }
    ...
    function storeExpandedNodes(nodes) {
        $.cookie('mptt_expanded_nodes', "[" + nodes.join(",") + "]", { expires: 7 });
    }

    function retrieveExpandedNodes() {
        var n = $.cookie('mptt_expanded_nodes');
        if (n !== null) {
            try {
                n = $.parseJSON(n);
            } catch(e) {
    ...
    function expandOrCollapseNode(item, store) {
        var show = true;
        ...
        if (store)
        {
            storeExpandedNodes(mptt.expanded_nodes);
        }
    ...
    $.extend($.fn.mpttTreeToggleItem = function() {
    ...
    if($('tbody tr', rlist).length > 1) {
        rlist.hide();
        $('tbody', rlist).mpttTree();
        $('span.page_marker', rlist).mpttTreeToggleItem();
        ...
        // collapse all nodes
        $('.page_marker', rlist).each(function()
        {
            expandOrCollapseNode($(this), false);
        });

        // uncollapse stored nodes        
        var storedNodes = retrieveExpandedNodes();
        if (storedNodes === null)
        {
        ...
        else 
        {
            for (var i=0; i < storedNodes.length; i++) 
            {
                ...
                expandOrCollapseNode($('#page_marker-' + storedNodes[i]), false);
            }
        }
    }
